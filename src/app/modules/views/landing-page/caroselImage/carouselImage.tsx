            ...
            {responsive.map((item) => (
              <div className="carouselContainer relative" key={item.id}>
                ...
                <div className="absolute top-0 left-0 right-0 bottom-0 flex flex-col items-center justify-center">
                  <div className="carouselText" style={{ backgroundColor: 'rgba(255, 255, 255, 0.65)', padding: '15px', borderRadius: '10px' }}>
                    <h1 className="text-deep-green font-bold text-2xl lg:text-5xl">{item.title}</h1>
                    <div className="text-green text-md lg:text-2xl font-semibold lg:mt-5">{item.text}</div>
                  </div>
                </div>
              </div>
            ))}
